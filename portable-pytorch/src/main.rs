...
#[clap(
    version = "1.0",
    author = "Noah Gift",
    about = "Command-line interface for ONNX resnet18 (portable-pytorch)"
)]
struct Cli {
    #[clap(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let args = Cli::parse();
    match args.command {
        Some(Commands::Infer { imagepath }) => {
            println!("imagepath: {}", imagepath);
            let img = image::open(imagepath).unwrap().to_rgb8(); //imagepath is the path to the image
            let model = tract_onnx::onnx()
                // load the model
                .model_for_path("resnet18-v1-7.onnx")?
